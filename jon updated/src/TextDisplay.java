...
public class TextDisplay {
    ...
    private static void CorrectionThread(){
        ...
        styledDocument.setCharacterAttributes(0, textbox.getText().length(), baseAttribute, true);
        for (Pair<Integer, Integer> location: locations){
            textbox.getStyledDocument().setCharacterAttributes(location.first, location.second, baseAttribute, true);
            int startIndex = location.first;
            int wordLength = location.second;
            String word = textbox.getText().substring(startIndex, startIndex + wordLength);
        // window
            // Dictionary.StartDictionary();
            // Check if the word is not in the dictionary before applying red styling
            if (!Dictionary.FindWord(word)) {
                System.out.println(word + "is red colored");
                StyleConstants.setForeground(redAttribute, Color.RED);
                textbox.getStyledDocument().setCharacterAttributes(startIndex,startIndex + wordLength, redAttribute, true);
            }
        }
    }
